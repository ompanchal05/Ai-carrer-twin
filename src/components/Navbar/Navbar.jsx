...
const APP_NAV = [
  ...
  { label: 'Job & Match', href: '/career-recommendation', icon: Briefcase },
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16 gap-4 sm:gap-6">
          ...
          {/* Center: Nav links - ICON ONLY as requested */}
                  ...
                  return (
                    <Link
                      ...
                      className={`relative p-2.5 rounded-xl transition-all duration-200 group flex items-center justify-center ${
                        active
                          ? 'bg-brand-500/15 text-brand-600 dark:text-brand-400 font-bold shadow-sm border border-brand-500/30'
                    ...
                    >
                      ...
                      {active && (
                        <span className="absolute bottom-1 w-1.5 h-1.5 rounded-full bg-brand-500 shadow-sm" />
                      )}

                      {/* Tooltip on hover */}
                      <span className="absolute top-full mt-2 px-2.5 py-1 rounded-lg bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-[10px] font-bold whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 shadow-xl z-50">
                        {label}
                      </span>
                    </Link>
                  );
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden">
                      ...
                      <div className="p-1.5 space-y-0.5">
                        ...
                        {((user?.email?.toLowerCase() === 'panchalom136@gmail.com') || (profile?.email?.toLowerCase() === 'panchalom136@gmail.com')) && (
                          <Link
                            to="/admin"
                            onClick={() => {
                              setShowUserMenu(false);
                          ...
                          >
                            <ShieldCheck className="w-4 h-4" /> Admin Portal (Om Only)
                          </Link>
                        )}
                        <Link
                          to="/settings"
